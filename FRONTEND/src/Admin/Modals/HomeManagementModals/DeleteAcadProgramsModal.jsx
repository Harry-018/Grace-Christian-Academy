...
import { deleteAcadPrograms } from "../../../services/service/acadProgramService";

const DeleteAcadProgramsModal = () => {
  const revalidator = useRevalidator();
  const closeDeleteAcadProgramsModal = WebManagementStore(
    (state) => state.closeDeleteAcadProgramsModal,
  );
  ...
  const handleDelete = async (e) => {
    ...
    try {
      ...
      await deleteAcadPrograms(selectedProgram.id);

      revalidator.revalidate();

      closeDeleteAcadProgramsModal();
      alert(`Successfully Deleted  ${selectedProgram.program}`);
    } catch (error) {
      console.error(error);
    } finally {
      setIsDeleting(false);
  ...
  return (
    <div className="fixed flex h-full w-full flex-col items-center justify-center bg-egg-dark/50 p-5">
      <div className="flex flex-col items-center gap-5 rounded-2xl bg-bone p-5 inset-shadow-med">
        ...
        >
          Are you sure you want to delete {selectedProgram.program} and it's
          data?
          <div className="flex gap-3">
            <button
              type="button"
              className="cursor-pointer rounded-lg p-2 text-sm duration-300 hover:opacity-75 active:scale-95"
              onClick={closeDeleteAcadProgramsModal}
              disabled={isDeleting}
            >
              Cancel
            </button>
...
export default DeleteAcadProgramsModal;
