...
import { createGrade } from "../../../services/service/tuitionService";

const CreateGradeModal = () => {
  const revalidator = useRevalidator();

  const closeCreateGradeModal = TuitionManagementStore(
    (state) => state.closeCreateGradeModal,
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(`Successfully Added  ${formData.grade_level}`);
    } catch (error) {
      console.error(error);
    } finally {
      setIsSaving(false);
    ...
    <div className="fixed flex h-full w-full flex-col items-center justify-center bg-egg-dark/50 p-5">
      <div className="flex flex-col items-center gap-5 rounded-2xl bg-bone p-5 inset-shadow-med">
        <h3 className="font-[PoppinsBold] text-swamp-green">
          Create Tuition for Grade Level
        </h3>
        <form
