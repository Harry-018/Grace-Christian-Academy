...
import { deleteTranspo } from "../../../services/service/transpoService";

const DeleteTranspoModal = () => {
  const revalidator = useRevalidator();
  const closeDeleteTranspoModal = TranspoManagementStore(
    (state) => state.closeDeleteTranspoModal,
  );
  ...
  const handleDelete = async (e) => {
    ...
    try {
      ...
      await deleteTranspo(selectedTranspo.transpo_id);

      revalidator.revalidate();

      closeDeleteTranspoModal();
      alert(`Successfully Deleted ${selectedTranspo.location}`);
    } catch (error) {
      console.error(error);
    } finally {
      setIsDeleting(false);
...
export default DeleteTranspoModal;
